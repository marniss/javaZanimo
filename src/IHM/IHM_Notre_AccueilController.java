...
public class IHM_Notre_AccueilController implements Initializable {
 ...
 /*
        ...
        if (IHM_loginController.membre == null) {
            btnsonne.setVisible(true);
        } else {
            ImageView imgp = new ImageView();
            File file = new File(IHM_loginController.membre.getPhoto());
            Image image = new Image(file.toURI().toString());
            imgp.setImage(image);
            imgp.setFitWidth(100);
            imgp.setFitHeight(100);
            maface.getChildren().add(imgp);
            maface.getChildren().add(new Label(IHM_loginController.membre.getNom()));
            maface.getChildren().add(new Label(IHM_loginController.membre.getType()));
            imgp.setVisible(true);
            Button logoutbtn = new Button("LogOut");
            logoutbtn.setOnMouseClicked(new EventHandler<Event>() {
                @Override
                public void handle(Event event) {
                    Parent root = null;
                    try {
                        root = FXMLLoader.load(getClass().getResource("IHM_Notre_Accueil.fxml"));
                    } catch (IOException ex) {
                        Logger.getLogger(IHM_Notre_AccueilController.class.getName()).log(Level.SEVERE, null, ex);
                    }
                    IHM_loginController.membre = null;
            ...
            maface.getChildren().add(logoutbtn);
            btnsonne.setVisible(false);
        }
    }
    ...
    private void onClickConcours(ActionEvent event) throws IOException {
        Parent root = FXMLLoader.load(getClass().getResource("VIEW_Concours.fxml"));
        Scene scene = new Scene(root);
        Stage stage = new Stage();
        stage.setScene(scene);
        stage.show();
    ...
    private void onClickGestionItilisateur(ActionEvent event) throws IOException {
        if (IHM_loginController.membre != null) {
            if (IHM_loginController.membre.getType().equals("Admin")) {
                Parent root = FXMLLoader.load(getClass().getResource("IHM_liste_membre.fxml"));
                Scene scene = new Scene(root);
                Stage stage = new Stage();
                stage.setScene(scene);
                stage.show();
            ...
            alert.setTitle("Eror");
            alert.setContentText("vous n'etes pas autoriser, il faut que vous soyer connecté en tant que 'Admin'");
            alert.show();
        }
    ...
    private void onClickProfil(ActionEvent event) throws IOException {
        Parent root = FXMLLoader.load(getClass().getResource("IHM_Profil.fxml"));
        Scene scene = new Scene(root);
        Stage stage = new Stage();
        stage.setScene(scene);
        stage.show();

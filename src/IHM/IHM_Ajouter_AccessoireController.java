...
public class IHM_Ajouter_AccessoireController implements Initializable {
    ...
    private void buttonValiderAction(ActionEvent event) {
        ServiceAccessoire temp = new ServiceAccessoire();
        IHM_Ajouter_AccessoireController.eror = temp.ajouterAcessoire(nom.getText(), description.getText(), prix.getText(), qte.getText(), type.getText(), photo.getText(), validite.getValue(), 1, categorie.getValue(), IHM_loginController.membre.getIdMembre());
        if (eror < 10) {
